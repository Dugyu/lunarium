...
import type { UseSliderProps, Writer, WriterRef } from './use-mts-slider';
...
function Slider({
  ...
  const rootStyleRef = useMainThreadRef<Record<string, string>>(
    initialRootStyle,
  );
  const trackStyleRef = useMainThreadRef<Record<string, string>>(
    initialTrackStyle,
  );
...
function SaturationSlider({
  ...
  const updateStyle = (next: WriteAction<Vec2>) => {
    ...
    if (resolved !== undefined) {
      const { edge: edgeBg, track: trackBg } = MTSHSLGradients
        .saturationGradientPair(resolved[0], resolved[1]);
      writeRootStyle.current?.({ 'background-image': edgeBg });
      writeTrackStyle.current?.({ 'background-image': trackBg });
    }
  };
...
function LightnessSlider({
  ...
  const updateStyle = (next: WriteAction<Vec2>) => {
    ...
    if (resolved !== undefined) {
      const { edge: edgeBg, track: trackBg } = MTSHSLGradients
        .lightnessGradientPair(resolved[0], resolved[1]);
      writeRootStyle.current?.({ 'background-image': edgeBg });
      writeTrackStyle.current?.({ 'background-image': trackBg });
    }
  };
