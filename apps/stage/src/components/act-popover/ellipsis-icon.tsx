...
const EllipsisIconPath = {
  sm: `path("${EllipsisIconPathData.sm}")`,
  md: `path("${EllipsisIconPathData.md}")`,
} as const;
...
function EllipsisIcon({ size = 'md', className }: EllipsisIconProps) {
  return (
    <view
      className={cn(
        size === 'sm' ? 'w-[18px] h-[18px]' : 'w-[24px] h-[24px]',
        'bg-neutral-content',
        className,
      )}
